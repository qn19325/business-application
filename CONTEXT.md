...
**Checklist item**:
A document slot on a tax return's collection checklist. One row per expected document (P60, bank statements, etc.). Marks whether Lara has received it (`done: boolean`). Items are auto-generated at tax return creation from regime-appropriate defaults, then pruned by Lara.
...
_Avoid_: authorisation (means HMRC agent authorisation, a separate concept), confirmation
